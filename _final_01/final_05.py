...
class edx(object):
    ...
    def setGrade(self, grade, course="6.01x"):
        ...
        try:
            assert 0 <= grade <= 100
            for curs in self.myCourses:
                if curs.courseName == course:
                    curs.setGrade(grade)
...
for c in edX.myCourses:
    ...
    print '%s:' % c.courseName,
    cn = c.courseName
    print edX.getGrade(cn)
    print 'Scores:',
    for pset in c.psetsDone:
        print pset[0], edX.getPset(pset[0], cn),
    print ''