...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        closest = self.points[0].distance(other.points[0])
        for self_point in self.points:
            for other_point in other.points:
                dist = self_point.distance(other_point)
                if dist < closest:
                    closest = dist
        return closest
    ...
    def maxLinkageDist(self, other):
        """ Returns the float distance between the points that
            ...
            self and the other point is from other. Uses the
            Euclidean dist between 2 points, defined in Point."""
        farthest = self.points[0].distance(other.points[0])
        for self_point in self.points:
            for other_point in other.points:
                dist = self_point.distance(other_point)
                if dist > farthest:
    ...
    def mysteryLinkageDist(self, other):
        """ Conceptually, the mysteryLinkage distance metric takes a vote
            for each data point and decides which cluster to assign
            the point to based on the majority vote.
        """
    ...
    def isIn(self, name):
        """ Returns True is the element named name
            is in the cluster and False otherwise """
        for p in self.points:
            if p.getName() == name:
                return True
        return False
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        """ Assumes clusters c1 and c2 are in self
            Assumes clusters are of type 'City'
            Adds to self a cluster containing the union of c1 and c2
            and removes c1 and c2 from self """
        tip = type(c1.points[0])
        points = []
        for p in c1.points:
            points.append(p)
        for p in c2.points:
            points.append(p)
        new_cluster = Cluster(points, tip)
        a, b = None, None
        for m in range(len(self.members)):
            if self.members[m] == c1:
                a = m
            if self.members[m] == c2:
                b = m
        del self.members[a]
        del self.members[b-1]  # decrement because of above deletion
    ...
    def findClosest(self, linkage):
        ...
        closest = linkage(self.members[0], self.members[1])
        for i in range(len(memb)-1):
            for j in range(i+1, len(memb)):
                dist = linkage(memb[i], memb[j])
                if dist < closest:
                    most_similar = (memb[i], memb[j])
                    closest = dist
        return most_similar
...
def hCluster(points, linkage, numClusters, printHistory):
    ...
    if printHistory:
        ...
        for i in range(len(history)):
            ...
            print()
    print('Final set of clusters: [ {} ]'.format(linkage.__name__))
    print(cS.toStr())
    return cS
...
def test_simple():
    ...
    points = buildCityPoints('test_exam.txt', False)
